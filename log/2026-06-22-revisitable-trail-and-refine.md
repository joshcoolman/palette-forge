...
What changed: the journey store now holds a *sequence* of variation rounds instead of one array. A refine bar (quick chips + free text) appends a steered round beneath the current one; re-picking a direction branches a fresh tail; the active path is marked "Exploring" on its direction card.
