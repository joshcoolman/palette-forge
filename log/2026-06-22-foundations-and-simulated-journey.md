...
- **Mechanism vs knowledge boundary for contrast.** Contrast *math* is locked in `contrast.ts`; *which* pairings and *what* AA/AAA targets to check is policy living in `knowledge/contrast.md` frontmatter, parsed to a typed `ContrastPolicy` fed to BOTH the (future) prompt and the code verifier. A welded `BASELINE` floor is enforced in code so a malformed or hostile markdown edit can't drop below AA or brick the app. This is the spec's "edit the knowledge, the output shifts" made concrete.
...
- For light-mode neutrals, surface needs *more* tint and a *bigger* lightness step than instinct suggests.
